...
class HeadTrackingSystem:
    ...
    def smooth_servo_move(self, servo, start_angle, end_angle):
        """
        Move the servo smoothly from start_angle to end_angle using small steps.
        """
        step = 1 if end_angle > start_angle else -1
        for angle in range(int(start_angle), int(end_angle), step):
            servo.write(angle)
            time.sleep(0.015)  # Small delay for smoother transition
        servo.write(end_angle)  # Ensure the servo reaches the exact target angle
        ...
        while self.running:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    # Map to servo positions with angle limits
                    target_x = max(50, min(130, 90 + (cx_nose - img_width // 2) // 10))
                    target_y = max(50, min(130, 90 - (cy_nose - img_height // 2) // 10))
